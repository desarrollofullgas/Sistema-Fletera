<x-app-layout>
    @section('title', 'UDT Eliminados')
    <x-slot name="header">
        <div>
            <x-card-greet-header>
                {{ __('Unidades Eliminadas') }}
            </x-card-greet-header>
        </div>
    </x-slot>
    <div class="content">
                                    ...
                                    @foreach ($trashed as $unidad)
                                        <tr
                                            ...
                                            <td
                                                ...
                                                <div class="text-sm">
                                                    <div class="font-medium text-gray-700 uppercase">{{ $unidad->name }}</div>
                                                </div>
                                            </td>
                                            ...
                                            <td
                                                class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b  block lg:table-cell relative lg:static">
                                                <span
                                                    class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold utpercase">Eliminado</span>
                                                {{ $ldd->deleted_at }}
                                            </td>
                                            <td
                                                class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                                                ...
                                                <div class="grid grid-cols-2">
                                                    <div>
                                                        ...
                                                        <form method="POST" action="{{ route('unidad_restore') }}"
                                                            onsubmit="return confirm('Restaurar Unidad');">
                                                            {{ csrf_field() }}
                                                            <input type="hidden" name="id"
                                                                value="{{ $unidad->id }}" required />
                                                            <button type="submit">
