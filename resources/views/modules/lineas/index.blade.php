<x-app-layout>
    @section('title', 'Líneas de transporte')
    <x-slot name="header">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <x-card-greet-header>
                {{ __('LÍNEAS DE TRANSPORTE') }}
            </x-card-greet-header>
            <div class=" flex justify-center space-x-2">
                <div>
                    @livewire('lineas.new-linea')
                </div>
            </div>
        </div>
    </x-slot>
                        ...
                        <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0 dark:bg-slate-800 dark:lg:hover:bg-slate-600">
                            ...
                            <th  class="w-full font-medium text-sm lg:w-auto p-3 text-gray-800 text-center border border-b dark:text-gray-400  dark:border-gray-700">
                                <div class="w-full flex justify-center gap-2">
                                    ...
                                    {{$linea->created_at->locale('es')->isoFormat('D / MMMM / YYYY H:mm:ss a')}}
                                </div>
                            </th>
                            <th  class="w-full font-medium text-sm lg:w-auto p-3 text-gray-800 text-center border border-b dark:text-gray-400  dark:border-gray-700">
                                <div class="w-full flex justify-center gap-2">
                                    @livewire('lineas.edit-linea',['lineaID'=>$linea->id])
                                    @livewire('lineas.delete-linea',['lineaID'=>$linea->id])
                                </div>
                            </th>
                        </tr>
                    @endforeach
