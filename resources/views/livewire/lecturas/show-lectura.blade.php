<x-modal-create tittle="" tipo="show" class="w-full">
    ...
    <x-slot name="content">
                ...
                @foreach ($lectura->detalles as $detalle)
                    <div class="w-full border-y py-2 ">
                        ...
                        <fieldset class=" border-t dark:border-gray-500">
                            ...
                            <div class="flex flex-wrap gap-4 justify-center">
                                <span><strong>Venta de perifericos:</strong> {{number_format($detalle->venta_periferico,2)}}</span>
                                <span><strong>Venta electrónica:</strong> {{number_format($detalle->venta_electronica,2)}}</span>
                                <span><strong>Odómetro:</strong> {{number_format($detalle->venta_odometro,2)}}</span>
                            </div>
                        </fieldset>
                        <br>
                        <fieldset class=" border-t dark:border-gray-500">
                            <legend class="mx-2 px-2 text-lg">Total Por Llenar</legend>
                            <div class="flex flex-wrap gap-4 justify-center text-red-500 font-bold">
                                <span><strong>Litros:</strong> {{number_format($detalle->veeder - $detalle->combustible->capacidad)}}</span>
                            </div>
                        </fieldset>
                    </div>    
                @endforeach
        ...
        $totalLitros = 0;
        foreach ($lectura->detalles as $detalle) {
            $totalLitros += $detalle->venta_electronica;
        }
        @endphp
        <div class="flex flex-wrap gap-4 justify-center mt-3">
            
            <span><strong>Total en litros:</strong> {{number_format($totalLitros,2)}}lts.</span>
