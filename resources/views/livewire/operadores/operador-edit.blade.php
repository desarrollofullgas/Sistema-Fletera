<div>
    ...
    <x-dialog-modal wire:model="EditOp" id="EditOp{{ $ope_id }}" class="flex items-center">
        ...
        <x-slot name="content">
            <div class="flex flex-wrap">
                ...
                <div class="mb-3 mr-2 col-6" wire:ignore>
                    <x-label value="{{ __('Licencia') }}" />
                    <x-input wire:model="licencia" class="{{ $errors->has('licencia') ? 'is-invalid' : '' }}"
                        type="text" name="licencia" :value="old('licencia')" required autofocus autocomplete="licencia" />
                    <x-input-error for="licencia"></x-input-error>
                </div>
            </div>
            <div class="flex flex-wrap">
                <div class="mb-3 mr-2 ">
                    ...
                    <x-input wire:model="rfc" class="{{ $errors->has('rfc') ? 'is-invalid' : '' }}" type="text"
                ...
                <div class="mb-3 mr-2 col-6" wire:ignore>
                    ...
                    <select id="status" wire:model="status"
                        ...
                        <option value="DISPONIBLE" @if ($status == 'DISPONIBLE') {{ 'selected' }} @endif>
                            DISPONIBLE</option>
                        <option value="EN VIAJE" @if ($status == 'EN VIAJE') {{ 'selected' }} @endif>
                            EN VIAJE</option>
                        <option value="EN DESCARGA" @if ($status == 'EN DESCARGA') {{ 'selected' }} @endif>
                            EN DESCARGA</option>
                        <option value="DESCARGA FINALIZADA"
                            @if ($status == 'DESCARGA FINALIZADA') {{ 'selected' }} @endif>
                            DESCARGA FINALIZADA</option>
                        <option value="VIAJE CANCELADO"
                            @if ($status == 'VIAJE CANCELADO') {{ 'selected' }} @endif>
                            VIAJE CANCELADO</option>
        ...
        <x-slot name="footer" class="d-none">
            <x-danger-button class="mr-2" wire:click="EditarOperador({{ $ope_id }})"
                wire:loading.attr="disabled">
                <div role="status" wire:loading wire:target="EditarOperador">
