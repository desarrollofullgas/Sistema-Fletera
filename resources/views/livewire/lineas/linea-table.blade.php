<div>
    <div class="py-4 space-y-4">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            {{-- Barra de Busqueda --}}
            <div>
                <x-input wire:model="search" type="text" class="w-auto" placeholder="Buscar lineas de transporte..." />
            </div>
            {{-- Acciones Masivas --}}
            @if ($checked)
                <x-dropdown align="right" width="60">
                    ...
                    <x-slot name="content">
                        <div class="w-60">
                            ...
                            <div>
                                @if ($valid->pivot->de == 1)
                                    <x-dropdown-link href="#" wire:click="deleteLineas">
                                        {{ __('Eliminar Linea Transporte') }}
                                    </x-dropdown-link>
                                @endif
        ...
        @if ($selectPage)
            @if ($selectAll)
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ $lineas->total() }}</strong> elementos.
                </div>
            @else
                <div class="text-gray-400 text-xs">
                    Se han seleccionado <strong>{{ count($checked) }}</strong> elementos, ¿Deseas seleccionar los
                    <strong>{{ $lineas->total() }}</strong>?
        ...
        <div class="flex-col space-y-4">
            ...
            <x-table class="hidden sm:table">
                <x-slot name="head">
                    ...
                    <x-heading sortable wire:click="sortBy('name')" :direction="$sortField === 'name' ? $sortDirection : null">TRANSPORTISTA</x-heading>
                    <x-heading sortable wire:click="sortBy('clave')" :direction="$sortField === 'clave' ? $sortDirection : null">CLAVE</x-heading>
                ...
                <x-slot name="body">
                    @forelse($lineas as $linea)
                        ...
                        <x-row wire:loading.class.delay="opacity-75">
                            {{-- Componente Column --}}
                            <x-cell> <x-input type="checkbox" value="{{ $linea->id }}" wire:model="checked" />
                            </x-cell>
                            <x-cell>{{ $linea->id }} </x-cell>
                            <x-cell><span class="font-bold">{{ $linea->name }}</span></x-cell>
                            <x-cell>{{ $linea->clave }} </x-cell>
                            <x-cell>{{ $linea->rfc}} </x-cell>
                            <x-cell> {{ $linea->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}
                            </x-cell>
                            <x-cell>
