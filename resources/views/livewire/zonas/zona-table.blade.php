            ...
            <div class="w-full flex flex-col gap-3 sm:hidden">
                @foreach ($zonas as $zona)
                    <div class="rounded-lg bg-white dark:bg-slate-700 shadow-sm flex flex-col gap-1">
                        <h3 class="font-semibold mb-2 bg-black dark:bg-dark-eval-0 text-gray-300 rounded-t-lg p-2">{{$zona->name}}</h3>
                        <div class="px-2 flex flex-col gap-2 text-sm">
                            ...
                            <p><strong>Fecha de registro: </strong>{{ $zona->created_at->locale('es')->isoFormat('D  MMMM  YYYY') }}</p>
                        </div>
                        <div class="flex gap-2 justify-center items-center">
                            <div>
                                @if ($valid->pivot->ed == 1)
